...
function init( ) {
  ...
  ddata.splitRecent = function splitRecent (time, cutoff, max) {
    var result = {
      first: { }
      , rest: { }
    };
  ...
  ddata.processDurations = function processDurations (treatments) {
  ...
  ddata.processTreatments = function processTreatments ( ) {
    ...
    // filter & prepare 'Insulin Change' events
    ddata.insulinchangeTreatments = ddata.treatments.filter( function filterMAGE (t) {
      return t.eventType.indexOf('Medtronic Insulin Change') > -1;
    }).sort(function (a,b) { return a.mills > b.mills; });
    
	// filter & prepare 'Insulin Change' events
    ddata.longActingTreatments = ddata.treatments.filter( function filterLAGE (t) {
      return t.eventType.indexOf('Long Acting Dose') > -1;
    }).sort(function (a,b) { return a.mills > b.mills; });
    ...
    ddata.tempbasalTreatments = ddata.processDurations(tempbasalTreatments);
    ...
    ddata.tempTargetTreatments = ddata.processDurations(tempTargetTreatments);
