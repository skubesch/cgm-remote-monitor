...
function init(ctx) {
  ...
  lage.getPrefs = function getPrefs(sbx) {
    ...
    return {
      ...
      , warn: sbx.extendedSettings.warn || 23
      , urgent: sbx.extendedSettings.urgent || 24
      , enableAlerts: sbx.extendedSettings.enableAlerts || true //changed from false
    };
  };
  ...
  lage.findLatestTimeChange = function findLatestTimeChange(sbx) {
    ...
    _.each(sbx.data.longActingTreatments, function eachTreatment (treatment) {
      ...
      if (treatmentDate > prevDate && treatmentDate <= sbx.time) {
        ...
        //var minutes = a.diff(b,'minutes') - hours * 60;
        ...
        if (!doseInfo.found || (age >= 0 && age < doseInfo.age)) {
          ...
          //
          doseInfo.display += doseInfo.minFractions + 'm';
        }
      }
    });
    ...
    var sound = 'incoming';
    var message;
    var sendNotification = true; //changed from false 
